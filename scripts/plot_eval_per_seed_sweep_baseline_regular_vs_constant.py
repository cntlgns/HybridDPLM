"""
Per-seed sweep plot comparing two uncond baseline LR variants:
  - REGULAR:  baseline-full-1e4_lr-fs0_uncond       / step_6971.0-loss_0.22
  - CONSTANT: baseline-full-1e4_lr-constant_uncond  / step_7176.0-loss_0.70

Layout follows plot_eval_per_seed_sweep_freeze_vs_redraw.py: linear-categorical
iter ticks, viridis ramp coloring annealing temperature, T values written next
to each point. Each iter slot is split horizontally into a REGULAR sub-slot
(left) and a CONSTANT sub-slot (right) so points never overlap.
...
OUT_DIR = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/baseline_regular_vs_constant")
...
MODELS = [
    ("baseline-full-1e4_lr-fs0_uncond",      "step_6971.0-loss_0.22", "REGULAR"),
    ("baseline-full-1e4_lr-constant_uncond", "step_7176.0-loss_0.70", "CONSTANT"),
]
...
def plot_one(records: pd.DataFrame, dataset: str, metric: str, agg: str,
    ...
    ax.set_xlabel("decoding iterations  (left sub-slot: REGULAR,  right sub-slot: CONSTANT)")
    ax.set_ylabel(f"{metric} ({agg} across proteins)")
    ax.set_title(f"baseline REGULAR vs CONSTANT (uncond) | {dataset} | {metric} | "
