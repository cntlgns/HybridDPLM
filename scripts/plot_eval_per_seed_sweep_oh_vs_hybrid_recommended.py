"""
Compare oh-xhigh (recommended decodings) vs hybrid plain (recommended).

oh-xhigh data: analysis/eval_per_seed_sweep/oh/summary.csv
emb-hybrid data: analysis/eval_per_seed_sweep/recommended/summary.csv
                  (label == "hybrid plain (recommended)")

oh-xhigh per-iter pick (chosen by lowest combined rank across the 16 cells
formed by {ca_rmsd, tm_score} x {cameo2022, PDB_date} x
{median, mean, best_median, best_mean} — i.e. typical AND oracle together,
not typical-median alone):
    iter 1   -> annealing0.5_0.1
    iter 3   -> annealing1.0_0.1   (manual override; lowest combined-rank
                                    pick was 0.5_0.1 but user requested 1.0_0.1)
...
OUT_DIR = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/oh_vs_hybrid_recommended")
...
HYBRID_COLOR = "#d62728"  # red (matches existing recommended/ plot)
...
def load_hybrid_series():
    df = pd.read_csv(REC_CSV)
    df = df[(df["label"] == HYBRID_LABEL) & (df["iter"].isin(ITERS))].copy()
    df["color"] = HYBRID_COLOR
...
def plot_one(records, dataset, metric, agg, out_path):
    ...
    for label in [OH_LABEL, HYBRID_LABEL]:
    ...
    ax.set_title(f"oh-xhigh vs emb-hybrid (recommended decodings) | {dataset} | "
                 f"{metric} | {agg} (filled) & best_{agg} (hollow)")
    ax.grid(True, alpha=0.3, axis="y")

    from matplotlib.lines import Line2D
    handles = [
        Line2D([0], [0], marker="o", color=OH_COLOR, markerfacecolor=OH_COLOR,
               markeredgecolor="black", markersize=9, label=OH_LABEL),
        Line2D([0], [0], marker="o", color=HYBRID_COLOR, markerfacecolor=HYBRID_COLOR,
               markeredgecolor="black", markersize=9, label=HYBRID_LABEL),
...
def main():
    ...
    records = pd.concat([load_oh_series(), load_hybrid_series()], ignore_index=True)
