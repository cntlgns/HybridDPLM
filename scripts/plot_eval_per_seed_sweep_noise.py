"""
Per-seed sweep plot for the noise model only:
  - noise-full-1e4_lr-fs0-cutL0_freeze / step_8202.0-loss_0.70

Generates one plot per (dataset, metric, aggregation) across all available
decoding-iter buckets. Layout mirrors plot_eval_per_seed_sweep_freeze_vs_redraw.py
without the model split: linear-categorical iter ticks on the x-axis, viridis
ramp coloring annealing temperature, T values written next to each point.

Datasets: cameo2022, PDB_date, cath_4.2_all, cath_4.3_all
Iters:    1, 3, 5, 10, 30, 100, 500  (500iter cath_4.3_all/annealing10.0_1.0
          is still running at the time of writing — gracefully skipped.)
Metrics:  ca_rmsd, bb_tmscore, inv_fold_seq_recovery, pmpnn_seq_recovery,
          mean_plddt
"""
import re
...
OUT_DIR = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/noise")

ITERS = [1, 3, 5, 10, 30, 100, 500]
DATASETS = ["cameo2022", "PDB_date", "cath_4.2_all", "cath_4.3_all"]

MODEL_DIR = "noise-full-1e4_lr-fs0-cutL0_freeze"
CKPT_STEM = "step_8202.0-loss_0.70"

# metric_name -> (csv_column, "min"|"max")  ("min" = lower is better)
METRICS = {
    "ca_rmsd":              ("ca_rmsd",              "min"),
    "bb_tmscore":           ("bb_tmscore",           "max"),
    "inv_fold_seq_recovery":("inv_fold_seq_recovery","max"),
    "pmpnn_seq_recovery":   ("pmpnn_seq_recovery",   "max"),
    "mean_plddt":           ("mean_plddt",           "max"),
}
...
def collect_records():
    ...
    for it in ITERS:
        ...
        if not root.exists():
            continue
        for ds in DATASETS:
            ckpt_dir = root / MODEL_DIR / ds / CKPT_STEM
            ...
            for dec_dir in sorted(ckpt_dir.iterdir()):
                ...
                for mname, (col, direction) in METRICS.items():
                    ...
                    per_protein_mean = grp.mean()
                    per_protein_best = grp.min() if direction == "min" else grp.max()
                    rows.append({
                        "iter":        it,
                        "dataset":     ds,
...
SLOT_TOTAL = 0.92  # horizontal width allotted to each iter slot
...
def plot_one(records: pd.DataFrame, dataset: str, metric: str, agg: str,
    ...
    for it, isub in sub.groupby("iter"):
        ...
        if n == 1:
            ...
            offsets = np.linspace(-SLOT_TOTAL / 2, SLOT_TOTAL / 2, n)
        ...
        for i, d in enumerate(decs):
            ...
            if d == "argmax":
                ...
                color = _decoding_color(d, anneal_n, anneal_decs.index(d))
    ...
    ax.set_title(f"NOISE freeze | {dataset} | {metric} | "
