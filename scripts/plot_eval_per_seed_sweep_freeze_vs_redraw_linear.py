"""
Per-seed sweep plot comparing two noise-linear decoding variants:
  - FREEZE: noise-full-1e4_lr-linear_freeze / step_9025.0-loss_0.22
  - REDRAW: noise-full-1e4_lr-linear_redraw / step_9025.0-loss_0.22

Mirrors plot_eval_per_seed_sweep_freeze_vs_redraw.py but for the lr-linear
schedule variant. Each iter slot is split horizontally into a FREEZE sub-slot
(left) and a REDRAW sub-slot (right) so points never overlap.
...
OUT_DIR = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/freeze_vs_redraw_linear")
...
MODELS = [
    ("noise-full-1e4_lr-linear_freeze", "step_9025.0-loss_0.22", "FREEZE"),
    ("noise-full-1e4_lr-linear_redraw", "step_9025.0-loss_0.22", "REDRAW"),
]
...
def plot_one(records: pd.DataFrame, dataset: str, metric: str, agg: str,
    ...
    ax.set_title(f"FREEZE vs REDRAW (lr-linear) | {dataset} | {metric} | "
