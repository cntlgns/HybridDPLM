"""
Per-seed sweep plot comparing two REDRAW noise variants:
  - REDRAW:        noise-full-1e4_lr-fs0-cutL0_redraw / step_8202.0-loss_0.70
  - REDRAW_LINEAR: noise-full-1e4_lr-linear_redraw   / step_9025.0-loss_0.22

Mirrors plot_eval_per_seed_sweep_freeze_vs_redraw_linear.py. Each iter slot is
split horizontally into a REDRAW sub-slot (left) and a REDRAW_LINEAR sub-slot
(right) so points never overlap.
...
OUT_DIR = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/redraw_vs_redraw_linear")
...
MODELS = [
    ...
    ("noise-full-1e4_lr-linear_redraw",    "step_9025.0-loss_0.22", "REDRAW_LINEAR"),
]
...
# width 0.44 with a 0.04 gap on each side of the slot center.
SLOT_TOTAL = 0.92
SUB_GAP = 0.04
...
def plot_one(records: pd.DataFrame, dataset: str, metric: str, agg: str,
    ...
    ax.set_xlabel("decoding iterations  (left sub-slot: REDRAW,  right sub-slot: REDRAW_LINEAR)")
    ax.set_ylabel(f"{metric} ({agg} across proteins)")
    ax.set_title(f"REDRAW vs REDRAW_LINEAR | {dataset} | {metric} | "
