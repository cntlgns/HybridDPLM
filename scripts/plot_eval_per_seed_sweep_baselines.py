"""
Per-seed sweep plot for the uncond baseline only.

For each (iter, decoding) we show one filled marker (typical aggregate over
proteins) and one hollow marker (oracle best-of-seeds), connected by a thin
vertical line. To keep multi-temp annealing readable we lay the iter axis on
linear-categorical ticks and spread the decodings horizontally within each
iter slot, color-coding annealing temperature on a viridis ramp and writing
the T values next to each point.
...
def _decoding_color(d, anneal_count, anneal_idx):
    ...
    # Clamp ramp into a readable mid-band of viridis.
...
def plot_one(records: pd.DataFrame, dataset: str, metric: str, agg: str,
    ...
    for it, isub in sub.groupby("iter"):
        ...
        for i, d in enumerate(decs):
            ...
            ax.scatter([x], [row[typical_col]], color=color, marker="o", s=85,
                       facecolors=color, edgecolors="black", linewidths=0.5,
                       zorder=3)
            ax.scatter([x], [row[best_col]], color=color, marker="o", s=85,
            ...
            # Label below the lower of the two markers so it doesn't sit
    ...
    # Faint vertical separators between iter slots so reading across is easy.
    for i in range(len(ITERS) - 1):
        ax.axvline(i + 0.5, color="0.85", linewidth=0.8, zorder=0)
    ...
    ax.set_title(f"baseline (uncond) | {dataset} | {metric} | "
