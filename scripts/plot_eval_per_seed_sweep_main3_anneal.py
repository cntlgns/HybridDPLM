"""
Same 3-model comparison as plot_eval_per_seed_sweep_main3.py
(baseline(uncond) vs hybrid(emb-high) vs noise), but plotting only the
annealing markers — both higher-T (^) and lower-T (v) per iter — and
dropping argmax.
"""
import os
import re
...
OUT_DIR = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/main3_anneal")
...
MODEL_COLORS = {
    "baseline-full-1e4_lr-fs0_uncond":             "#1f77b4",
...
def build_marker_map(records: pd.DataFrame) -> dict:
    """Higher annealing T -> '^', lower annealing T -> 'v'. argmax and any
    other decodings are dropped (returned as None)."""
    out = {}
    for it, sub in records.groupby("iter"):
        ...
        for d in decs:
            if anneal_temps and d == anneal_temps[-1][0]:
                out[(it, d)] = "^"
...
def plot_one(records: pd.DataFrame, dataset: str, metric: str, agg: str,
    ...
    ax.set_title(f"main3_anneal | {dataset} | {metric} | "
    ...
    leg3 = ax.legend(handles=style_handles, title="style", loc="upper left",
                     bbox_to_anchor=(1.02, 0.20), borderaxespad=0,
                     frameon=True, fontsize=9)
    ax.add_artist(leg1)
    ax.add_artist(leg2)
    fig.tight_layout()
