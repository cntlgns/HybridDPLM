"""
Per-protein IQR (across 10 seeds) aggregated across proteins, vs BoN(N=10).

For each (label, iter, dataset, metric, agg):
  * For each protein, compute typical (mean or median) and Q1, Q3 of the metric
    across the 10 seeds.
  * Aggregate across proteins with `agg`:
      - bold line  = agg(per-protein typical)
      - low band   = agg(per-protein Q1)
      - high band  = agg(per-protein Q3)
      - BoN dotted = agg(per-protein min/max across seeds)

This contrasts with the seed_iqr version (sibling script): there the IQR is
across seed-level averages and is therefore very narrow (since each seed's
average over many proteins is similar). Here the IQR is per-protein
seed-variance averaged across proteins, which captures how much a single
sampling decision moves a single protein's metric.

Two pairwise comparisons (sharing baseline):
  - oh_vs_baseline:  hybrid(one-hot)  vs  baseline(disc. FT)
  - emb_vs_baseline: hybrid(embed)    vs  baseline(disc. FT)
...
OUT_ROOT = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/"
                f"oh_vs_hybrid_vs_baseline_protein_iqr")
...
def _per_protein_stats(df: pd.DataFrame, metric: str, agg: str):
    """Return DataFrame indexed by protein_id with cols typ, q1, q3, bon."""
    ...
    bon = grp.agg(fn_bon)
    out = pd.DataFrame({"typ": typ, "q1": q1, "q3": q3, "bon": bon})
    return out
...
def plot_one(records, comparison_labels, dataset, metric, agg, out_path):
    ...
    for label in comparison_labels:
        ...
        ax.plot(xs, typs, color=color, linewidth=2.6, zorder=3,
    ...
    handles += [
        Line2D([0], [0], color="k", linewidth=2.6,
