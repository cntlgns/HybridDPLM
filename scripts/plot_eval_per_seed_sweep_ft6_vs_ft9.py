"""
Per-seed sweep plot comparing two emb-high checkpoints:
  - FT6: emb-high_noise-full-ema_lr_745-fs0-normemb / step_9005.0-loss_0.67
  - FT9: emb-high_noise-full-1e4_lr               / step_9843.0-loss_0.68

Layout follows plot_eval_per_seed_sweep_baselines.py: linear-categorical iter
ticks, viridis ramp coloring annealing temperature, T values written next to
each point. To fit two models per iter we split each iter slot horizontally
into a FT6 sub-slot (left) and a FT9 sub-slot (right) so points never overlap.
...
OUT_DIR = Path(f"{PROJECT_DIR}/analysis/eval_per_seed_sweep/ft6_vs_ft9")
...
MODELS = [
    ("emb-high_noise-full-ema_lr_745-fs0-normemb", "step_9005.0-loss_0.67", "FT6"),
    ("emb-high_noise-full-1e4_lr",                 "step_9843.0-loss_0.68", "FT9"),
]
...
def plot_one(records: pd.DataFrame, dataset: str, metric: str, agg: str,
    ...
    ax.set_xlabel("decoding iterations  (left sub-slot: FT6,  right sub-slot: FT9)")
    ax.set_ylabel(f"{metric} ({agg} across proteins)")
    ax.set_title(f"FT6 vs FT9 | {dataset} | {metric} | "
